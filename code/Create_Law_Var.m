...
if ( strcmp(Def_Base.RTM,'FLIGHT') )
    ...
    for n = 1:nspecies
            ...
            LawsFlight(:,n,1) = 0.001 + LawsFlight(:,n,1) .* ( LAI_Crown >= Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).LAI_min) .* (LAI_Crown <= Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).LAI_max);
            ...
            LawsFlight(:,n,8) = getHeight(Law.Age,LawsFlight(:,n,2),...
                            ...
                            Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).H_a3], ...
           ...
           LawsFlight(:,n,10) = getrxy(LawsFlight(:,n,10), ...
                           [Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).rxy_c0 ,...
                            Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).rxy_c1], ...
                            Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).rxy_LB , ...
                            Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).rxy_UB , ...
                            Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).rxy_model);   
              
            LawsFlight(:,n,12) = 0.5*LawsFlight(:,n,8).*(Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).rz_d0 + LawsFlight(:,n,10).*Def_Base.(['Class_' num2str(Class)]).FLIGHT(n).rz_d1);
    ...
    ind = nspecies-sum(LawsFlight(:,:,1) > repmat(rand(Nb_Sims,1),1,nspecies),2 )+1;
    ...
    Law.Max_ht = min(Law.H-1.3,max(1.3,Law.H - 2* Law.rz) +  LawsFlight(sub2ind(size(LawsFlight),(1:length(ind))',ind,9+0*ind)));
    Law.Min_ht = max(1.3,max(1.3,Law.H -2* Law.rz) -  LawsFlight(sub2ind(size(LawsFlight),(1:length(ind))',ind,9+0*ind)));
