        ...
        try
            ...
            switch Def_Base.Regression.(Def_Base.Algorithm_Name).Method
                case {'NNT}'
                        ...
                        if ( strcmp(P,'Single')  )
                        %% not cascading
                        [Results.(Def_Base.Algorithm_Name).(P),Perf_Theo.(Def_Base.Algorithm_Name).(P)]= Train_NNT_Sim_batch([Def_Base.Var_out],Input_Noise,Output,Regression,Class,5);
                        
                        else
                            %% cascading
                            [Results.(Def_Base.Algorithm_Name).Single,Perf_Theo.(Def_Base.Algorithm_Name).Single]= Train_NNT_Sim_batch(P,Input_Noise,Output,Regression,Class,5);
                            ...
                            [Results.(Def_Base.Algorithm_Name).(P),Perf_Theo.(Def_Base.Algorithm_Name).(P)]= Train_NNT_Sim_batch_P([Def_Base.Var_out], Input_Noise,Output,Regression,Class,5,Plist);
                        end
                otherwise
                    if ( Debug )
...
%                         do validation and plot results
                        ...
                        Plot_Perfo_Theo(Def_Base,ResultsActual,Perf_Actual,Class,[Def_Base.Validation_name '_' Def_Base.Algorithm_Name '_' P],'');

                        %%incertitudes based on the validation results
                        % determine RMSE for each output variable for input sample 
                         [RMSEI]= RMSEIncertitude(Def_Base,Perf_Theo,Input,Output)
                         [ResultsActualI,Perf_ActualI]= Train_NNT_Sim_batch([Def_Base.Var_out],Input_Noise,RMSEI,Regression,Class,5)
  
                      Plot_Perfo_Theo(Def_Base,ResultsActualI,Perf_ActualI,Class,['Incertitudes' Def_Base.Validation_name '_' Def_Base.Algorithm_Name  '_' P],'-Error');
