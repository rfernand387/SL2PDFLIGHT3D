function [Results,Perf_Theo]= Train_NNT_Sim_batch_testing(Output_Name,Input, Output,Regression,Cats,numBatches,maxSims)
...
% subsampling uniformly to maxSims
In=[];
for ivar = 1:length(Input_Name)
    In=cat(2,In,Input.(Input_Name{ivar})(1:floor(Nb_Sims/maxSims):Nb_Sims,:));
end
Cats = Cats(1:floor(Nb_Sims/maxSims):Nb_Sims);
Nb_Sims = length(Cats);
...
for ivar=1:length(Output_Name)
    ...
    % Format output
    Out = (Output.(Output_Name{ivar})(1:floor(Nb_Sims/maxSims):Nb_Sims,:))';
    N = length(Out);
    ...
    for b = 1:numBatches
        ...
        for ires=1:Regression.Var_out.(Output_Name{ivar}).Nb_Reseau
            ...
            % Train the Network
            [net,tr] = train(net,In,Out,'useParallel','no','useGPU','no');
            temp = net(In);
